...
/**
 * PrivacyEmbedRationalOptionPages class
 * 
 * renamed to PrivacyEmbedRationalOptionPages to comply wordpress requirements concerning "generic function/class/define/namespace names" (26.05.2021)
 * added some escape functions to comply wordpress requirements concerning "data must be sanitized, escaped, and validated" (08.06.2021)
 *
 ...
 * @version		1.0.0
 */
class PrivacyEmbedRationalOptionPages
{
  /* ==========================================================================
	   Vars
	   ========================================================================== */
...
?> <script>
          ...
          if (!empty($section_params['fields'])) {
            foreach ($section_params['fields'] as $field_key => $field_params) {
              ...
              if (
                ...
                $params['title'] = '<label for="{$params["id"]}">' . __($params['title'], 'privacy-embed') . '</label>';
              }
  ...
  protected function build_menu_page($page_key)
  {
    ...
    settings_errors();
    ?><div class="wrap">
      <h1><?php _e($GLOBALS['title'], 'privacy-embed'); ?></h1><?php
          ...
          switch ($field['type']) {
            case 'checkbox':
              ...
              printf(
                ...
                $field['value'],                                  // value
                !empty($field['text']) ? esc_html(__($field['text'], 'privacy-embed')) : ''                    // text
              );
              break;
            case 'media':
              ...
              printf(
                ...
                $field['id'],                                    // id
                "{$page_key}[{$field['id']}]",                            // name
                !empty($field['placeholder']) ? 'placeholder="' . __($field['placeholder'], 'privacy-embed') . '"' : '',    // placeholder
                $field['title_attr'],                                // title
                $field['value'],                                  // value
                !empty($attributes) ? implode(' ', $attributes) : '',              // additional attributes
                $upload_button,                                    // upload button
                !empty($field['text']) ? '<p class="help">' . esc_html(__($field['text'], 'privacy-embed')) . '</p>' : ''        // text
              );
              break;
            case 'radio':
              echo '<fieldset><legend class="screen-reader-text">' . esc_html(__($field['title'], 'privacy-embed')) . '</legend>';
              $c = 0;
              foreach ($field['choices'] as $value => $label) {
                ...
                printf(
                  ...
                  $field['id'],                                    // id
                  "{$page_key}[{$field['id']}]",                            // name
                  esc_attr(__($label, 'privacy-embed')),                                        // title
                  esc_attr($value),                                        // value
                  esc_html(__($label, 'privacy-embed')),                                        // label
            ...
            case 'select':
              ...
              printf(
                ...
                $field['id'],                                    // id
                $field_tag_name,                            // name
                __($field['title_attr'], 'privacy-embed')                                // title
              );
              foreach ($field['choices'] as $value => $text) {
                ...
                printf(
                  ...
                  $selected,                                      // selected
                  esc_attr($value),                                        // value
                  esc_html(__($text, 'privacy-embed'))                                        // text
                );
              }
              echo '</select>';
              break;
            case 'textarea':
              printf(
                ...
                $field['title_attr'],                                // title
                esc_textarea($field['value']),                                  // value
                !empty($field['text']) ? '<p class="help">' . esc_html(__($field['text'], 'privacy-embed')) . '</p>' : ''        // text
              );
              break;
            case 'wp_editor':
              $field['textarea_name'] = "{$page_key}[{$field['id']}]";
              wp_editor(isset($field['value']) ? esc_textarea($field['value']) : '', $field['id'], array(
                'textarea_name'    => $field['textarea_name'],
              ));
              echo !empty($field['text']) ? '<p class="help">' . esc_html(__($field['text'], 'privacy-embed')) . '</p>' : '';
              break;
            default:
              printf(
        ...
        protected function build_settings_section($page_key, $section_key)
        {
          ...
          echo !empty($section['text']) ? esc_html($section['text']) : '';
        ...
        protected function validate_field($field, $page_key, $section_key, $field_key, $page, $section)
        {
          ...
          if (empty($field['title'])) {
            $this->submit_error(__('Field parameter "title" is required', 'privacy-embed'));
          }
          ...
          // Title attribute
          $field['title_attr'] = empty($field['title_attr']) ? __($field['title'], 'privacy-embed') : $field['title_attr'];
        ...
        protected function validate_page($page_key, $page_params, $parent_slug = false)
        {
          ...
          if (empty($page_params['page_title'])) {
            $this->submit_error(__('Page parameter "page_title" is required', 'privacy-embed'));
          }

          // Menu title
          if (empty($page_params['menu_title'])) {
            $page_params['menu_title'] = __($page_params['page_title'], 'privacy-embed');
          }
        ...
        protected function validate_section($section, $page_key, $section_key, $page)
        {
          ...
          if (empty($section['title'])) {
            $this->submit_error(__('Section parameter "title" is required', 'privacy-embed'));
          }
